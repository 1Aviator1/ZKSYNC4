...
func (h *HardKey) GetLedger() *ledger.Ledger {
	return h.l
}
...
func parseLedgerErr(err error, fallback string) {
	...
	case strings.Contains(errString, "LedgerHID device") && strings.Contains(errString, "not found"):
		ux.Logger.PrintToUser(logging.Red.Wrap("ledger is not connected"))
	case strings.Contains(errString, "6b0c"):
		ux.Logger.PrintToUser(logging.Red.Wrap("ledger is not unlocked"))
	case strings.Contains(errString, "APDU_CODE_CONDITIONS_NOT_SATISFIED"):
		ux.Logger.PrintToUser(logging.Red.Wrap("ledger rejected signing"))
	default:
		ux.Logger.PrintToUser(logging.Red.Wrap("%s: %v"), fallback, err)
	}
}
...
func retriableLegerAction(f func() error, fallback string) error {
	for {
		...
		ux.Logger.PrintToUser(logging.Cyan.Wrap("ledger action failed..."))
...
func NewHard(networkID uint32) (*HardKey, error) {
	k := &HardKey{}
	ux.Logger.PrintToUser(logging.Yellow.Wrap("connecting to ledger..."))
	...
	ux.Logger.PrintToUser(logging.Yellow.Wrap("deriving address from ledger..."))
	...
	ux.Logger.PrintToUser(logging.Yellow.Wrap("derived primary address from ledger: %s"), k.pAddrs[0])
	return k, nil
}
...
func (h *HardKey) C() string   { return "not implemented" }
...
func (h *HardKey) Spends(outputs []*avax.UTXO, opts ...OpOption) (
	...
	for _, out := range outputs {
		...
		if err != nil {
			ux.Logger.PrintToUser(logging.Yellow.Wrap("cannot spend with current key: %s"), err)
			continue
		}
